...
v3=v(index_zero4:length(v));
I3=I(index_zero4:length(v));
...
lgd=legend;
lgd.FontSize=14;
lgd.Title.String='number of cycles' ;
...
text(0.1,0.1,Area,'Units','normalized','Color','red','FontSize',12)

scan_rate=string(round((v(1)-v(6))/(file.t(1)-file.t(6)),2));
scan=append('scan rate = ',scan_rate,' V/s');
text(0.1,0.15,scan,'Units','normalized','Color','blue','FontSize',12)

saveas(gcf, strcat(file.folder,'\Figures\regions\',extractBefore(file.name,".csv"),".png" ))
saveas(gcf, strcat(file.folder,'\Figures\regions\',extractBefore(file.name,".csv"),".m" ))

figure(2)
semilogy(v1,abs(I1),'-o','LineWidth',2,'MarkerSize',2)
hold on
semilogy(v2,abs(I2),'-o','LineWidth',2,'MarkerSize',2)
hold on
semilogy(v3,abs(I3),'-o','LineWidth',2,'MarkerSize',2)
hold off
...
saveas(gcf, strcat(file.folder,'\Figures\regions\logy\',extractBefore(file.name,".csv"),".png" ))
saveas(gcf, strcat(file.folder,'\Figures\regions\logy\',extractBefore(file.name,".csv"),".m" ))
