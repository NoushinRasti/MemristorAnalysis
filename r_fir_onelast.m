...
if (length(counter)>2 )
vol=file.v(counter(len-1): counter(len));
curr=file.I(counter(len-1): counter(len));
...
text(0.5,0.15,scan,'Units','normalized','Color','blue','FontSize',12)
...
xlabel('Voltage(V)')
ylabel('Current(A)')
lgd=legend({'1',lastone}, 'Location','northwest');
lgd.Title.String='number of measurement' ;
...
saveas(gcf, strcat(file.folder,'\Figures_first_lastone\4plot\IV\',extractBefore(file.name,".csv"),".png" ))
 hold off
