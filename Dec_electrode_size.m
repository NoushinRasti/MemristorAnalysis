...
path="/Users/noushinrasti/Desktop/Imperial_Work/Lab_work/data_1year_memristor/PHD_WELCOME/data_befor_10Dec/results/data/MAPI_5Dec/7dec/1-thin-perov-500/data";
title_figure="ITO/PEDOT:PSS/MAPI/Ag";
...
%   data=readmatrix(strcat(files(index).folder,'/',files(index).name));
    full_path = strcat(files(index).folder,'/',files(index).name,'/',files(index).name,'-',range_voltage,'-',sr,'.csv');
...
text(0.5,0.15,'scan rate = 0.25 V/s','Units','normalized','Color','blue','FontSize',15)
